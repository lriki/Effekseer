...
ModelLoader::ModelLoader(LLGI::G3::Graphics* graphics, ::Effekseer::FileInterface* fileInterface )
	: graphics			( graphics )
	...
	if( reader.get() != NULL )
	{
		...
		for (int32_t f = 0; f < model->GetFrameCount(); f++)
		{
...

			{
				...
				model->InternalModels[f].IndexBuffer = graphics->CreateIndexBuffer(
					4,
					sizeof(int32_t) * 3 * model->InternalModels[f].FaceCount * model->ModelCount);
